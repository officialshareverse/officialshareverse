...
class CreateGroupSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if mode == "sharing":
            if not access_identifier or not access_password:
                raise serializers.ValidationError({
                    "access_password": "Sharing groups must include the login email or username and password."
                })
        else:
            if access_identifier or access_password or attrs.get("access_notes", ""):
                raise serializers.ValidationError({
